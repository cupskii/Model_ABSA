...
Kredensial (MLFLOW_TRACKING_URI, kunci R2) diambil dari Modal Secret
...
def train_remote(model_config: dict, data: dict, git_commit: Optional[str] = None,
    ...
    git_commit   : str, opsional — commit hash dihitung di mesin pemanggil
                   (laptop), bukan di sini: image ini tidak menyertakan
                   direktori .git maupun binary git, jadi dihitung ulang
                   di dalam kontainer akan selalu salah/'unknown'.
    run_id       : str, opsional — MLflow run ID yang sudah dibuka pemanggil
                   (mis. run_experiment.py) untuk dilanjutkan di kontainer
                   ini, alih-alih membuat run baru — lihat run_train_model().
    ...
    train_result = run_train_model(model_config, data, git_commit=git_commit, run_id=run_id)
...
def main(config_path: str = "configs/experiment_indobert_baseline.yaml"):
    ...
    from run_experiment import get_git_commit
    ...
    # Dihitung di laptop (yang punya .git sungguhan) — bukan di dalam
    # kontainer Modal, lihat catatan di train_remote().
    git_commit = get_git_commit()

    print("Memanggil GPU Modal untuk training ...")
    train_result = train_remote.remote(model_config, data, git_commit)
